  ...
  def update(self, CC, CC_SP, CS, now_nanos):
    ...
    if (self.frame % self.p.STEER_STEP) == 0:
      ...
      if self.CP.flags & SubaruFlags.LKAS_ANGLE:
        apply_torque = apply_std_steer_angle_limits(actuators.steeringAngleDeg, self.apply_torque_last, CS.out.vEgoRaw, self.p)

        if not CC.latActive:
          apply_torque = CS.out.steeringAngleDeg

        can_sends.append(subarucan.create_steering_control_angle(self.packer, apply_torque, CC.latActive))
      else:
    ...
    if self.CP.flags & SubaruFlags.LKAS_ANGLE:
      new_actuators.steeringAngleDeg = self.apply_torque_last
    else:
